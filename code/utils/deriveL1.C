...
void deriveL1(string input_path, string algo, string algo2, string era)
{
  ...
  cout << "Reading root input file from " << path << endl
       << flush;
  ...
  TFile *f = new TFile(Form("%s/Parameter_%s.root", path, a));
  assert(f && !f->IsZombie());
  ...
  for (int ieta = 0; ieta != neta; ++ieta)
  {
    ...
    if (!batch && fabs(eta + 2.0) > 0.1)
      continue;

    string s = Form("p_offOverA_EtaVsRhoVsJetPt/"
                    "p_offOverA_RhoVsJetPt_%d",
                    ieta + 1);
    cout << s << endl
         << flush;
    ...
    if (!batch)
      cout << Form("fmu: chi2/NDF = %1.1f / %d\n",
                   fmu1->GetChisquare(), fmu1->GetNDF())
           << endl;

    if (!batch)
      c1->SaveAs("pdf/deriveL1_offOverA_vs_rho.pdf");
    ...
    if (!batch)
      cout << Form("fpt: chi2/NDF = %1.1f / %d\n",
                   fpt->GetChisquare(), fpt->GetNDF())
           << endl;

    if (!batch)
      c2->SaveAs("pdf/deriveL1_offOverA_vs_pT_clean.pdf");
  ...
  if (batch)
    c4->SaveAs(Form("deriveL1_filtered_%s.pdf", a));
